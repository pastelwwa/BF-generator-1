...
// --- Fonty / interlinia ---
const FONT1_PX = 60; // duży (TT-Travels)
const FONT2_PX = 30; // mały (TT-Commons)
const FONT3_PX = 25; // mniejszy (TT-Commons, tylko M)
...
function applySharpen(srcCanvas, amountPct){
  const amount=Math.max(0,Math.min(2,amountPct/100)); if(amount===1) return srcCanvas;
  ...
  for(let y=1;y<h-1;y++){ for(let x=1;x<w-1;x++){ const idx=(y*w+x)*4;
    for(let ch=0;ch<3;ch++){ let acc=0;
  ...
  sctx.putImageData(dst,0,0); return srcCanvas;
}
function computeBaseScaleForImage(img){
  const ang=state.imgAngle%4, iw=(ang%2===0)?img.width:img.height, ih=(ang%2===0)?img.height:img.width;
...
function render(){
  ...
  if(state.img && state.ramka){
    ...
    octx.save(); octx.translate(cx,cy); octx.rotate(ang*Math.PI/2);
...
function bindUI(){
  ...
  el('autoFitBtn').addEventListener('click', ()=>{ if(!state.img) return;
    state.baseScale=computeBaseScaleForImage(state.img);
    state.zoomExtra=0; el('zoomExtra').value=0;
    state.offx=0; state.offy=0; el('offx').value=0; el('offy').value=0; render();
  });
  el('rotateBtn').addEventListener('click', ()=>{ if(!state.img) return;
    state.imgAngle=(state.imgAngle+1)%4; state.baseScale=computeBaseScaleForImage(state.img); render();
  });
  el('resetCorrBtn').addEventListener('click', ()=>{ state.bright=100; state.sat=100; state.cont=100; state.sharp=100;
    ['bright','sat','cont','sharp'].forEach(id=>el(id).value=100); render();
  });
  ...
  c.addEventListener('mousemove', e=>{
    ...
    el('offx').value=state.offx; el('offy').value=state.offy; render();
  });
  ...
  // Gesty dotyk
  let touchDragging=false,startX=0,startY=0,baseOffX=0,baseOffY=0; let pinching=false,startDist=0,baseZoomExtra=0;
  function dist(t1,t2){ return Math.hypot(t2.clientX-t1.clientX,t2.clientY-t1.clientY); }
  c.addEventListener('touchstart', e=>{
    e.preventDefault();
    if(e.touches.length===1){ touchDragging=true; pinching=false; startX=e.touches[0].clientX; startY=e.touches[0].clientY; baseOffX=state.offx; baseOffY=state.offy; }
    else if(e.touches.length===2){ pinching=true; touchDragging=false; startDist=dist(e.touches[0],e.touches[1]); baseZoomExtra=state.zoomExtra; }
  }, {passive:false});
  c.addEventListener('touchmove', e=>{
    ...
    if(touchDragging && e.touches.length===1){
      ...
      const scaleX=W/c.clientWidth, scaleY=H/c.clientHeight;
      state.offx=Math.round(baseOffX+dx*scaleX); state.offy=Math.round(baseOffY+dy*scaleY);
      el('offx').value=state.offx; el('offy').value=state.offy; render();
    }else if(pinching && e.touches.length===2){
      const d=dist(e.touches[0],e.touches[1]); const ratio=d/Math.max(1,startDist);
